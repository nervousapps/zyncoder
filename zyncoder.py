...
def lib_zyncoder_init():
    global lib_zyncoder
    try:
        lib_zyncoder = cdll.LoadLibrary(
            dirname(realpath(__file__))+"/build/libzyncoder.so")
        lib_zyncoder.init_zynlib()
        # Setup return type for some functions
        lib_zyncoder.get_midi_filter_clone_cc.restype = ndpointer(
            dtype=c_ubyte, shape=(128,))
